 ...
 */
public class WordSelectionFilter extends FilterAdapter implements Example,
		SpecialBehavior {
	...
	public String getFilterName() {
		...
		return Text.get(Text.FILTER_WORD_SELECTION_LABEL);
	}
	...
	public String getSmallDescription() {
		...
		return Text.get(Text.FILTER_WORD_SELECTION_TOOLTIP);
	}
	...
	public String getFilterInstructions() {
		...
		return Text.get(Text.FILTER_WORD_SELECTION_INSTRUCTIONS);
	}
	...
	protected StringBuffer filter(StringBuffer text, StringBuffer auxiliarText)
		...
		if ("".equals(text.toString().trim())) { //$NON-NLS-1$
			JOptionPane.showMessageDialog(getSuperComponent(), Text
					.get(Text.FILTER_CHARACTER_SELECTION_NO_CONTENT_MESSAGE),
					Text.get(Text.FILTER_CHARACTER_SELECTION_NO_CONTENT_TITLE),
					JOptionPane.OK_OPTION);
			return new StringBuffer();
		}

		WordSelectionDialog dialog = new WordSelectionDialog(
				Text.get(Text.FILTER_WORD_SELECTION_TITLE), text, false);

		StringBuffer output = new StringBuffer();

		for (ArrayList<SelectedElement> line : dialog.getLines()) {
			for (SelectedElement element : line) {

				if (element.isSelectedWord()) {
					output.append(element.getText());
				} else {
					int spaces = element.getText().length();
	...
	public String getMainInputExample() {
		...
		return Text.get(Text.FILTER_WORD_SELECTION_EXAMPLE_MAIN_INPUT);
	}
	...
	public String getOutputExample() {
		...
		return Text.get(Text.FILTER_WORD_SELECTION_EXAMPLE_OUTPUT);
	}
	...
	public Behavior[] getSpecialBehavior() {
		...
		return new Behavior[] { Behavior.WORK_ONLY_ON_TEXT };
	}
