...
/**
 * Swap contents on the line based on parameter.
 * 
 * @author Carlos Fernando Bella Cruz - <a
 *         href="mailto:pessoal@carlosbcruz.com">pessoal@carlosbcruz.com</a>
 */
public class SwapContentBasedOnParameterFilter extends FilterAdapter implements
		CommandLine, SpecialBehavior, Example, Report {

	private static final long serialVersionUID = 1L;

	private int contentsSwaped = 0;
	...
	public String getFilterName() {
		...
		return Text.get(Text.FILTER_SWAPCONTENTBASEDONPOSITION_LABEL);
	}
	...
	public String getSmallDescription() {
		...
		return Text.get(Text.FILTER_SWAPCONTENTBASEDONPOSITION_TOOLTIP);
	}
	...
	public String getFilterInstructions() {
		...
		return Text.get(Text.FILTER_SWAPCONTENTBASEDONPOSITION_INSTRUCTIONS);
	}
	...
	public String[] getControlsLabels() {
		...
		return new String[] {
				Text.get(Text.FILTER_SWAPCONTENTBASEDONPOSITION_FIELD1),
				Text.get(Text.FILTER_SWAPCONTENTBASEDONPOSITION_CHECKBOX1) };
	}
	...
	public String[] getControlsToolTipTexts() {
		...
		return new String[] {
				Text.get(Text.FILTER_SWAPCONTENTBASEDONPOSITION_FIELD1_TOOLTIP),
				Text.get(Text.FILTER_SWAPCONTENTBASEDONPOSITION_CHECKBOX1_TOOLTIP) };
	}
	...
	protected StringBuffer filter(StringBuffer text, StringBuffer auxiliarText)
		...
		this.contentsSwaped = 0;

		if ("".equals(getField1())) { //$NON-NLS-1$
			throw new FilterException(
					Text.get(Text.FILTER_SWAPCONTENTBASEDONPOSITION_EXCEPTION));
		}
		...
		while (tokenizer.hasMoreTokens()) {
			...
			int index = isCheckBox1() ? token.indexOf(getField1()) : token
					.toUpperCase().indexOf(getField1().toUpperCase());
			...
			if (index == -1) {
				...
				String secondPart = token.substring(index
						+ getField1().length(), token.length());

				this.contentsSwaped++;
				if (tokenizer.isFoundNewLine()) {
					output.append(secondPart + getField1() + firstPart
							+ getNewLine());
				} else {
					output.append(secondPart + getField1() + firstPart);
				}
	...
	public String getHelpDescription() {
		...
		return Text
				.get(Text.FILTER_SWAPCONTENTBASEDONPOSITION_COMMAND_LINE_HELP);
	}
	...
	public String getMainInputExample() {
		...
		return Text
				.get(Text.FILTER_SWAPCONTENTBASEDONPOSITION_EXAMPLE_MAIN_INPUT);
	}
	...
	public String getOutputExample() {
		...
		return Text.get(Text.FILTER_SWAPCONTENTBASEDONPOSITION_EXAMPLE_OUTPUT);
	}
	...
	public String getReportText() {
		...
		return Text.get(Text.FILTER_SWAPCONTENTBASEDONPOSITION_REPORT,
				String.valueOf(this.contentsSwaped));
	}
}
