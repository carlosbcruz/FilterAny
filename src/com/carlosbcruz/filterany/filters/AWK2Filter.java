 ...
 */
public class AWK2Filter extends FilterAdapter implements CommandLine,
		SpecialBehavior, Example {

	private static final long serialVersionUID = 1L;

	private static final String INPUT_FILE = "AWK2in.txt"; //$NON-NLS-1$
	private static final String OUTPUT_FILE = "AWK2out.txt"; //$NON-NLS-1$
	private static final String OUTPUT_FINISHED_FLAG_FILE = "AWKScript2.awk.ok"; //$NON-NLS-1$
	private static final String SCRIPT_FILE = "AWKScript2.awk"; //$NON-NLS-1$
	private static final String BATCH_FILE = "awk.bat"; //$NON-NLS-1$
	...
	public String getFilterName() {
		...
		return Text.get(Text.FILTER_AWK2FILTER_LABEL);
	}
	...
	public String getSmallDescription() {
		...
		return Text.get(Text.FILTER_AWK2FILTER_TOOLTIP);
	}
	...
	public String getFilterInstructions() {
		...
		return Text.get(Text.FILTER_AWK2FILTER_INSTRUCTIONS);
	}
	...
	protected StringBuffer filter(StringBuffer text, StringBuffer auxiliarText)
		...
		// Write input to file.
		FileSupport.writeTextFile(INPUT_FILE, text.toString());

		// Execute script.
		try {
			Runtime.getRuntime().exec(
					BATCH_FILE + " " + SCRIPT_FILE + " " + INPUT_FILE + " " //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
							+ OUTPUT_FILE);
		} catch (IOException exception) {
			throw new FilterException(Text.get(
					Text.FILTER_AWK2FILTER_EXCEPTION1, exception.getMessage()));
		}

		// Wait for the script to end by monitoring the flag file.
		int timeout = 0;
		boolean foundFinishFile = false;
		File file = new File(OUTPUT_FINISHED_FLAG_FILE);
		do {
			try {
				Thread.sleep(500);
			} catch (InterruptedException exception) {
				throw new FilterException(Text.get(
						Text.FILTER_AWK2FILTER_EXCEPTION1,
						exception.getMessage()));
			}

			if (file.exists()) {
				foundFinishFile = true;
			}

			timeout++;

		} while (!foundFinishFile && timeout < TIMEOUT);
		// Delete flag file.
		file.delete();
		...
		// If timeout then try to delete output and throw exception.
		if (timeout >= TIMEOUT) {
			...
			throw new FilterException(
					Text.get(Text.FILTER_AWK2FILTER_EXCEPTION2));
		}

		// Retrieve the output file
		StringBuffer output = new StringBuffer();

		output.append(FileSupport.readTextFile(OUTPUT_FILE));
	...
	public String getHelpDescription() {
		...
		return Text.get(Text.FILTER_AWK2FILTER_COMMAND_LINE_HELP);
	}
	...
	public String getMainInputExample() {
		...
		return Text.get(Text.FILTER_AWK2FILTER_EXAMPLE_MAIN_INPUT);
	}
	...
	public String getOutputExample() {
		...
		return Text.get(Text.FILTER_AWK2FILTER_EXAMPLE_OUTPUT);
	}
