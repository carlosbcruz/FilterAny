...
/**
 * Combine fields into a string and validate the procedure against a mask.
 * 
 * @author Carlos Fernando Bella Cruz - <a
 *         href="mailto:pessoal@carlosbcruz.com">pessoal@carlosbcruz.com</a>
 */
public class CombineFieldsIntoStringFilter extends FilterAdapter implements
	...
	public String getFilterName() {
		...
		return Text.get(Text.FILTER_COMBINEFIELDSINTOSTRINGFILTER_LABEL);
	}
	...
	public String getSmallDescription() {
		...
		return Text.get(Text.FILTER_COMBINEFIELDSINTOSTRINGFILTER_TOOLTIP);
	}
	...
	public String getFilterInstructions() {
		...
		return Text.get(Text.FILTER_COMBINEFIELDSINTOSTRINGFILTER_INSTRUCTIONS);
	}
	...
	public String[] getControlsLabels() {
		...
		return new String[] { Text
				.get(Text.FILTER_COMBINEFIELDSINTOSTRINGFILTER_TARGET1) };
	}
	...
	public String[] getControlsToolTipTexts() {
		...
		return new String[] { Text
				.get(Text.FILTER_COMBINEFIELDSINTOSTRINGFILTER_TARGET1_TOOLTIP) };
	}
	...
	protected StringBuffer filter(StringBuffer text, StringBuffer auxiliarText)
		...
		try {
			...
			throw new FilterException(Text.get(
					Text.FILTER_COMBINEFIELDSINTOSTRINGFILTER_EXCEPTION1,
		...
		for (String line : lines) {
			...
			if ("".equals(line)) { //$NON-NLS-1$
				throw new FilterException(Text.get(
						Text.FILTER_COMBINEFIELDSINTOSTRINGFILTER_EXCEPTION2,
		...
		while (tokenizer.hasMoreTokens()) {
			...
			// The field starts on next character.
			startCharacter++;
			...
			if (startCharacter == -1 || endCharacter == -1
				...
				throw new FilterException(Text.get(
						Text.FILTER_COMBINEFIELDSINTOSTRINGFILTER_EXCEPTION3,
						String.valueOf(lineNumber), token));

			}

			// Get the field.
			String thisField = token.substring(startCharacter, endCharacter);

			// Get the field description from mask.
			if (fieldCounter >= fields.size()) {
				throw new FilterException(Text.get(
						Text.FILTER_COMBINEFIELDSINTOSTRINGFILTER_EXCEPTION4,
						String.valueOf(lineNumber), token));
			}

			FieldsBean field = fields.get(fieldCounter);

			if (field.getSize() != thisField.length()) {
				throw new FilterException(Text.get(
						Text.FILTER_COMBINEFIELDSINTOSTRINGFILTER_EXCEPTION5,
						field.getFieldDescription(), token,
						String.valueOf(field.getSize())));
			}

			output.append(thisField);
		...
		if (fieldCounter < fields.size()) {
			...
			throw new FilterException(Text.get(
					Text.FILTER_COMBINEFIELDSINTOSTRINGFILTER_EXCEPTION6,
	...
	public String getHelpDescription() {
		...
		return Text
				.get(Text.FILTER_COMBINEFIELDSINTOSTRINGFILTER_COMMAND_LINE_HELP);
	}
	...
	public String getMainInputExample() {
		...
		return Text
				.get(Text.FILTER_COMBINEFIELDSINTOSTRINGFILTER_EXAMPLE_MAIN_INPUT);
	}
	...
	public String getOutputExample() {
		...
		return Text
				.get(Text.FILTER_COMBINEFIELDSINTOSTRINGFILTER_EXAMPLE_OUTPUT);
	}
