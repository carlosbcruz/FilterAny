 ...
 */
public class MultipleInsertAuxiliarIntoMainFilter extends FilterAdapter
	...
	public String getFilterName() {
		...
		return Text.get(Text.FILTER_MULTIPLEINSERTAUXILIARINTOMAIN_LABEL);
	}
	...
	public String getSmallDescription() {
		...
		return Text.get(Text.FILTER_MULTIPLEINSERTAUXILIARINTOMAIN_TOOLTIP);
	}
	...
	public String getFilterInstructions() {
		...
		return Text
				.get(Text.FILTER_MULTIPLEINSERTAUXILIARINTOMAIN_INSTRUCTIONS);
	}
	...
	public String[] getControlsLabels() {
		...
		return new String[] { Text
				.get(Text.FILTER_MULTIPLEINSERTAUXILIARINTOMAIN_FIELD_1) };
	}
	...
	public String[] getControlsToolTipTexts() {
		...
		return new String[] { Text
				.get(Text.FILTER_MULTIPLEINSERTAUXILIARINTOMAIN_FIELD_1_TOOLTIP) };
	}
	...
	protected StringBuffer filter(StringBuffer text, StringBuffer auxiliarText)
		...
		// Retrieve the step.
		String stepParameter = getField1().trim();

		int step = 1;
		if (!"".equals(stepParameter)) { //$NON-NLS-1$

			try {
				step = Integer.parseInt(stepParameter);
			} catch (NumberFormatException e) {
				throw new FilterException(
						Text.get(
								Text.FILTER_MAINTAINLINESATREGULARSTEPFILTER_EXCEPTION1,
								stepParameter));
			}
			if (step <= 0) {
				throw new FilterException(
						Text.get(
								Text.FILTER_MAINTAINLINESATREGULARSTEPFILTER_EXCEPTION2,
								stepParameter));
			}
		}
		...
		do {
			...
			for (int i = 0; i < step; i++) {
				if (mainTokenizer.hasMoreTokens()) {
					mainLine += mainTokenizer.nextToken() + getNewLine();
					mainLinesInserted++;
				}
			}

			if (mainLinesInserted == step) {
				output.append(mainLine + auxiliarText.toString());
				this.numberOfInsertions++;
			} else {
				output.append(mainLine);
			}

		} while (mainLinesInserted == step);
	...
	public String getHelpDescription() {
		...
		return Text
				.get(Text.FILTER_MULTIPLEINSERTAUXILIARINTOMAIN_COMMAND_LINE_HELP);
	}
	...
	public String getAuxiliarInputExample() {
		...
		return Text
				.get(Text.FILTER_MULTIPLEINSERTAUXILIARINTOMAIN_EXAMPLE_AUXILIAR_INPUT);
	}
	...
	public String getMainInputExample() {
		...
		return Text
				.get(Text.FILTER_MULTIPLEINSERTAUXILIARINTOMAIN_EXAMPLE_MAIN_INPUT);
	}
	...
	public String getOutputExample() {
		...
		return Text
				.get(Text.FILTER_MULTIPLEINSERTAUXILIARINTOMAIN_EXAMPLE_OUTPUT);
	}
	...
	public String getReportText() {
		...
		return Text.get(Text.FILTER_MULTIPLEINSERTAUXILIARINTOMAIN_REPORT,
				String.valueOf(this.numberOfInsertions));
	}
}
